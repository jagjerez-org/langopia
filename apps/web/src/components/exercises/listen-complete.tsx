...
import { Headphones, RotateCcw, Eye } from "lucide-react";
...
export function ListenComplete({ exercise, mode }: Props) {
  ...
  return (
    <div className="rounded-lg border bg-card p-4 space-y-3">
      <div className="flex items-center gap-2 text-cyan-600 dark:text-cyan-400">
        ...
        <span className="text-xs font-medium uppercase tracking-wide">Listen & Complete</span>
      </div>
      ...
      <div className="space-y-2">
        {lines.map((line, lineIdx) => {
          ...
          return (
            <div
              key={lineIdx}
              className={`flex gap-2 ${speaker === "B" ? "justify-end" : ""}`}
            >
              <div
              ...
              >
                {isBlank ? (
                  <>
                    {line.replace(/^[A-Z]:\s*/, "").split("___").map((part, pi, arr) => (
                      <span key={pi}>
                        {part}
                        {pi < arr.length - 1 && (
