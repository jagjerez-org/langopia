...
/** El enlace de Meet no expira por nuestra cuenta; esto solo fuerza a volver a preguntar si algo cambió. */
const ANSWER_TTL_MS = 60 * 60 * 1000;

/**
 * Google Meet necesita que la escuela conecte su cuenta de Google Workspace
 * por OAuth para poder programar la reunión en su nombre — esa conexión no
 * existe todavía en este monorepo (mismo tipo de deuda que
 * `ZoomRoomAdapter`, documentada allí con más detalle).
 *
 ...
 * Meet es abrir su enlace con la cuenta de Google de quien se conecta.
 */
@Injectable()
export class GoogleMeetRoomAdapter implements RoomProviderPort {
  constructor(@Inject(CLOCK) private readonly clock: Clock) {}

  async createRoom(_request: CreateRoomRequest): Promise<RoomCreationResult> {
    return {
      status: "pending",
      reason: "La escuela todavía no tiene Google Meet conectado (OAuth pendiente).",
    };
  }
