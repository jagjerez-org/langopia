...
/**
 * Esquema de la salida estructurada del corrector.
 *
 * `byCriterion` puntúa cada criterio de 0 a 5 —no de 0 a 100—: es la escala
 * que espera `Rubric.weightedScore` para llegar a la nota final sobre el
 * `maxScore` del ejercicio. Se comprueba contra las claves reales de la
 * rúbrica en el adaptador (no aquí: este esquema no conoce la rúbrica de la
 * llamada concreta, solo la forma genérica «cadena → número»).
 */
...
export function buildWritingCorrectionPrompt(params: {
  task: string;
  response: string;
  rubric: { criteria: ReadonlyArray<{ key: string; label: string; descriptors: readonly string[] }> };
  language: string;
  level: string;
  ...
  const user =
    ...
    `Rúbrica (evalúa cada criterio por separado, en una escala de 0 a 5):\n${criteriaText}\n\n` +
    ...
    `arriba (las mismas claves, ni una de más ni de menos), cada una con su nota de 0 a 5`;
