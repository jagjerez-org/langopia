...
import { SubmitExamCommand } from "./submit-exam.command.js";

/** El alumno entrega el examen. La corrección (`grade-exam`) es un paso aparte, explícito. */
...
export class SubmitExamHandler implements ICommandHandler<SubmitExamCommand> {
  ...
  async execute(command: SubmitExamCommand): Promise<{ examId: string; status: string }> {
    ...
    const exam = await this.uow.execute(async () => {
      ...
      found.submit({ responses: command.props.responses, now });
      await this.exams.save(found);
      return found;
    });

    await this.events.publish(exam.pullDomainEvents());
    return { examId: exam.id.value, status: exam.status };
  }
}
